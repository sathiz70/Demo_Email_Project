...
const localLogin = new LocalStrategy(localOptions, (req, email, password, done) => {
  User.findOne({ email }, (err, user) => {
    ...
    if (!user) {
      return done(respondError(req.__(localesKeys.auth.PLEASE_REGISTER), constValues.StatusCode.NOT_FOUND), false);
    }
    ...
    user.comparePassword(password, (error, isMatch) => {
      ...
      if (!user.status) {
        done(respondError(req.__(localesKeys.auth.USER_NOT_ACTIVE), constValues.StatusCode.CONFLICT), false);
      }
